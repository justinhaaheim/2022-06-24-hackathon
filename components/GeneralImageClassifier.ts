...
// class label
export default async function classifyImage(image: ImagePT) {
  ...
  if (model == null) {
    console.log('Checking cache for image classifier model...');
    const filePath = await ModelCache.getModelPath('imageClassifier');
    console.log('Loading image classifier model...');
    model = await torch.jit._loadForMobile(filePath);
    console.log('Image classifier downloaded and loaded.');
  }
